function test_unified_periodic_orbit_entrypoint()
%TEST_UNIFIED_PERIODIC_ORBIT_ENTRYPOINT Regression checks for the unified PO_extract entrypoint.

po_dir = fileparts(mfilename('fullpath'));
addpath(po_dir);
matcontRoot = fullfile(fileparts(po_dir), 'MatCont7p6');
...
assert(vdp_result.solver_used == "direct", 'Default solver route should remain direct.');
...
unsupportedOptionCases = { ...
    'min_events', 9; ...
    'match_tol', 1e-3; ...
    'period_tol', 2e-3; ...
    'backend', 'matcont'; ...
    'unsupported_flag', true};
for k = 1:size(unsupportedOptionCases, 1)
    unsupportedOpts = opts;
    unsupportedOpts.(unsupportedOptionCases{k, 1}) = unsupportedOptionCases{k, 2};
    unsupportedResult = extract_periodic_orbit(@vdp_rhs, [2; 0], 1, unsupportedOpts);
    assert(unsupportedResult.status == "invalid_options", ...
        'Unsupported options should report invalid_options.');
    assert(strcmp(char(unsupportedResult.message), 'Unsupported options in opts.'), ...
        'Unsupported options should use the generic rejection message.');
end
...
assert(~isfield(api_result.diagnostics, 'quality'), ...
    'diagnostics.quality should stay disabled by default.');
...
quality_opts = api_opts;
quality_opts.computeQualityDiagnostics = true;
quality_result = extract_periodic_orbit(@vdp_rhs, [2; 0], 1, quality_opts);
assert(isfield(quality_result.diagnostics, 'quality'), ...
    'computeQualityDiagnostics=true should populate diagnostics.quality.');
...
handles = adaptx();
adaptx_rhs = @(t, y, p) handles{2}(t, y, p(1), p(2));
matcont_opts = struct( ...
    'orbit_solver', 'matcont', ...
    'tspan', [0, 300], ...
    'transientFraction', 0.8, ...
    'matcont_root', matcontRoot, ...
    'matcont_odefile', @adaptx, ...
...
matcont_result = extract_periodic_orbit(adaptx_rhs, [0.3; 0.5; -0.1], [1; 0.8], matcont_opts);
assert(matcont_result.success && matcont_result.solver_used == "matcont", ...
    'orbit_solver=matcont should forward directly to the MATCONT backend.');

removed_direct_then_opts = matcont_opts;
removed_direct_then_opts.orbit_solver = 'direct_then_matcont';
removed_direct_then_result = extract_periodic_orbit(adaptx_rhs, [0.3; 0.5; -0.1], [1; 0.8], removed_direct_then_opts);
assert(removed_direct_then_result.status == "invalid_options", ...
    'Removed direct_then_matcont route should report invalid_options.');
assert(contains(char(removed_direct_then_result.message), '''direct'' or ''matcont'''), ...
    'Removed direct_then_matcont route should now fall through to the generic solver validation message.');

removed_internal_opts = matcont_opts;
removed_internal_opts.orbit_solver = 'matcont_internal';
removed_internal_result = extract_periodic_orbit(adaptx_rhs, [0.3; 0.5; -0.1], [1; 0.8], removed_internal_opts);
assert(removed_internal_result.status == "invalid_options", ...
    'Removed matcont_internal route should report invalid_options.');
assert(contains(char(removed_internal_result.message), '''direct'' or ''matcont'''), ...
    'Removed matcont_internal route should now fall through to the generic solver validation message.');
