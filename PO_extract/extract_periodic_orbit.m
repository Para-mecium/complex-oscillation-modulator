...
try
    ...
    result.diagnostics.matcont = build_matcont_diagnostics(opts);
...
if any(result.code == [1, 2])
    ...
    [result.max_variable, result.min_variable, result.amplitude] = compute_statistics(orbitStruct.y);
end
...
if isempty(opts.minCrossings)
    if ~isempty(opts.min_events)
        opts.minCrossings = max(2, double(opts.min_events));
    else
        opts.minCrossings = 6;
    end
end

if isempty(opts.poincareTol) && ~isempty(opts.match_tol)
    opts.poincareTol = opts.match_tol;
elseif ~isempty(opts.match_tol)
    opts.poincareTol = opts.match_tol;
end

if isempty(opts.periodTol) && ~isempty(opts.period_tol)
    opts.periodTol = opts.period_tol;
elseif ~isempty(opts.period_tol)
    opts.periodTol = opts.period_tol;
end
...
function [max_variable, min_variable, amplitude] = compute_statistics(orbit_y)
if isempty(orbit_y)
    max_variable = [];
    min_variable = [];
    amplitude = [];
...
max_variable = max(orbit_y, [], 1);
min_variable = min(orbit_y, [], 1);
amplitude = (max_variable - min_variable) / 2;
end

function diagnostics = build_matcont_diagnostics(opts)
requested = false;
backend = "";
if isstruct(opts) && isfield(opts, 'backend') && ~isempty(opts.backend)
    backend = string(opts.backend);
    requested = strcmpi(char(backend), 'matcont');
end

matcontFields = {'matcont_root', 'matcont_odefile', 'matcont_active_parameter', ...
    'matcont_ntst', 'matcont_ncol', 'matcont_tolerance'};
for k = 1:numel(matcontFields)
    name = matcontFields{k};
...
diagnostics = struct( ...
    ...
    'requested', requested, ...
    'backend', backend, ...
    'reason', "MATCONT refinement is disabled in this implementation.");
end
...
result = struct( ...
    ...
    'period', [], ...
    'orbit_t', [], ...
    'orbit_y', zeros(0, n_state), ...
    'amplitude', [], ...
    'max_variable', [], ...
    'min_variable', [], ...
    'event_times', zeros(0, 1), ...
    'event_states', zeros(0, n_state), ...
    'backend_used', "direct", ...
    'orbit', struct(), ...
    'diagnostics', struct(), ...
    'raw', struct());
