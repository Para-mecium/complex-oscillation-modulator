function fig = plot_cancer_normal_form(data, opts)
%PLOT_CANCER_NORMAL_FORM Plot precomputed cancer normal-form curves.
...
xlabel(ax, 'f_{11}');
ylabel(ax, 'f_{12}');
title(ax, sprintf('AM: max |\\Delta\\omega| = %.3e', checks.am_omega_max_abs));
...
if cfg.show_constraint_plot
    ...
    legend(ax, {'AM', 'FM', 'f_{21}=(a_{21}/a_{12})f_{12}'}, 'Location', 'best');
end
end

function opts = default_options()
opts = struct();
opts.figure_name = 'Cancer normal-form curves';
