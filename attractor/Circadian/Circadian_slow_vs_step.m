% Circadian oscillator: compare slow parameter modulation and a sudden jump.
...
pathFile = fullfile(repoDir, 'Circadian', 'data', 'fig5b', 'curves', ...
    'fig5b_iso_period_curve_T24p0.mat');
pathBranchName = 'lowerRightBranch';
...
[p0, p1, pathLabel] = select_circadian_path(pathData, pathBranchName);
...
if ~initialOrbit.success
    error('Circadian_slow_vs_step:InitialOrbitFailed', ...
...
function [p0, p1, pathLabel] = select_circadian_path(pathData, branchName)
if ~isfield(pathData, 'lowerSeed') || ~isfield(pathData, branchName)
    error('select_circadian_path:InvalidPathData', ...
        'Expected lowerSeed and branch %s in the selected path file.', branchName);
end

branch = pathData.(branchName);
if isempty(branch)
    error('select_circadian_path:EmptyBranch', 'Selected branch is empty.');
end

p0 = reshape(pathData.lowerSeed.Parameters, 1, []);
p1 = reshape(branch(end).params, 1, []);
pathLabel = sprintf('fig5b T24p0: lowerSeed -> %s(end)', branchName);
end
