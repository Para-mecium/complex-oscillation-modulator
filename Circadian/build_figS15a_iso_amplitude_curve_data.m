...
for i = 1:numel(targetAmplitudes)
    ...
    seedTask = solve_seed_task(baseSolverView, targetAmplitude, centerPeriod, controlledIdx, errBound, derivatives, sys);
    seed = export_seed_data(seedTask, baseY0);

    leftBranchTask = solve_branch_task( ...
        seedTask.exportSolverView(), targetAmplitude, leftKd, curveStepCap, controlledIdx, errBound, derivatives, sys);
    rightBranchTask = solve_branch_task( ...
        seedTask.exportSolverView(), targetAmplitude, rightKd, curveStepCap, controlledIdx, errBound, derivatives, sys);
...
%%
function task = solve_seed_task(baseSolverView, targetAmplitude, centerPeriod, controlledIdx, errBound, derivatives, sys)
...
%%
function task = solve_branch_task(seedSolverView, targetAmplitude, targetKd, curveStepCap, controlledIdx, errBound, derivatives, sys)
continuationOptions = struct( ...
