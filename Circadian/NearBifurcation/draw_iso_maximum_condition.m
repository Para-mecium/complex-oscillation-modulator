...
for i = 1:numel(curveData)
    curve = curveData{i};
    curve = downsample_curve(curve, maxLinePoints);
    surface(ax, ...
...
function curve = downsample_curve(curve, maxPoints)
[curve.Kd, curve.AT, curve.logCondition] = downsample_line( ...
    curve.Kd, curve.AT, curve.logCondition, maxPoints);
end
