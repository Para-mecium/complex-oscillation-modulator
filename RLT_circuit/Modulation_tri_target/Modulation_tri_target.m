function result = Modulation_tri_target(varargin)
%   MODULATION_TRI_TARGET Build self-contained tri-target modulation caches.
%   MODULATION_TRI_TARGET() generates reusable periodic-orbit caches for
%   the 1.0x, 1.5x, and 2.0x period targets starting from the inferred
%   parameters in learnedData_ODE.mat.
%
%   MODULATION_TRI_TARGET('generatePathCache', true) also generates the
...
for i = 1:2:numel(varargin)
    ...
    switch lower(name)
        ...
        otherwise
            error('Modulation_tri_target:UnknownOption', ...
...
ctx.baseVarAmp = baseState.varAmp;
...
for i = 2:numel(targetMultipliers)
    ...
    stateViewInput = fmam_state_ops.solverViewFromState(stateObj);
    [items_per, items_controlled, ~, periodTarget] = ...
        build_tri_target_setup(stateObj, ctx.basePeriod, ctx.baseVarAmp, periodMultiplier);

    errBound = 1e-6;
    continuationOptions = struct('initialLambdaStep', 0.01);

    task = FMAM_ODE(ctx.sys, ctx.obs, stateViewInput, items_per, items_controlled, ...
    ...
    if ~poResult.has_orbit
        error('Modulation_tri_target:PeriodicOrbitGenerationFailed', ...
...
if needPath
    stateObj = create_state(ctx);
    stateViewInput = fmam_state_ops.solverViewFromState(stateObj);
    [items_per, items_controlled, ~] = build_tri_target_setup( ...
        stateObj, ctx.basePeriod, ctx.baseVarAmp, 2);
    ...
    solution = task.logs;
    if isempty(solution)
        error('Modulation_tri_target:MissingContinuationLogs', ...
...
function [items_per, items_controlled, max_stepsize, periodTarget] = ...
        build_tri_target_setup(stateObj, basePeriod, baseVarAmp, periodMultiplier)
    ...
    items_per(2).idx = 1;
    items_per(2).target = baseVarAmp(1);
    
    items_per(3).prop = 'varAmp';
    items_per(3).idx = 2;
    items_per(3).target = baseVarAmp(2);
    ...
    targetDelta = [ ...
        ...
        baseVarAmp(1) - stateObj.varAmp(1), ...
        baseVarAmp(2) - stateObj.varAmp(2)];
