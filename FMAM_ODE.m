classdef FMAM_ODE < handle
    properties 
        ...
        minLr = 5e-2 % minimum learning rate
        lr % learning rate  
        isPsiUpdated = false 
    ...
    methods
        ...
        function val = res(obj)
            ...
            TS_Dvar = -(vs.*(1:M))*p_var(2:end,:) + (vc(:,2:end).*(1:M))*q_var;
        ...
        function oneIter(obj)
            ...
            for j = 1:obj.n_per
                ...
                if any([strcmpi(propname,'p_Psi'),strcmpi(propname,'q_Psi'),strcmpi(propname,'p_var'), ...
                    ...
                    A(idxLastRow+1, indexMap.(propname)(idx_target)) = 1;
                    res(idxLastRow+1) = 0;
            ...
            if obj.isPsiUpdated
                if strcmpi(PV.name,'var')
                    ...
                    A(idxLastRow + (1:M-1),idx_p) = diag(ones(M-1,1));
                    res(idxLastRow + (1:M-1)) = -p_var(3:end,idx_PV);

                    idxLastRow = idxLastRow + M-1;

                    A(idxLastRow + (1:M),idx_q) = diag(ones(M,1));
    ...
    methods(Static)
        ...
        function [coe_para,coe_p_Psi,coe_q_Psi,coe_p_var,coe_q_var] = ...
         ...
         vc,vs,idx_eq)
            ...
            coe_p_Psi = -F(TS_variable,parameters).*vc;       
            coe_q_Psi = -F(TS_variable,parameters).*vs; 
